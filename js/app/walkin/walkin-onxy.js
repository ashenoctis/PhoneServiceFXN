...
export class Walkin_onxy extends Component {
  ...
  componentDidMount() {
   ...
   $('.pagenotfounds').hide()
    ...
    axios.get(apilink+'/website/walkin_time/', JSON.stringify(axiosConfig))
      .then(response => {
  ...
  handleAddJob(e) {
    ...
    console.log(cookies.get('device-issues'))
      ...
      axios.post(apilink+'/website/makepayment/', {id: cookies.get('userId'),products: cookies.get('products') ,part_code: cookies.get('part-code'), amount: cookies.get('total-estimate'),location: this.state.deliveryCentre, "date" : cookies.get('date_slot'), "time" : cookies.get('time_slot')}, JSON.stringify(axiosConfig))
      .then(response => {
          ...
          <Visible xs sm>
            <Container fluid className="header header-mobile">
              <Row id="floating-btn-bot" className="floating-btn-bot-row hide">
                <Col lg={12} md={12} sm={12} xs={12}>
                  <FlatButton onClick={this.handleAddJob} id= "desk-btn-mobile-enable" label={(cookies.get('skip-pay') === '1') ? 'CONFIRM ORDER' : 'PAY ₹ ' + cookies.get('total-estimate')} fullWidth className="floating-btn-bot paybutton"/>
