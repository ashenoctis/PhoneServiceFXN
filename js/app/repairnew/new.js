...
let flag = false;
let max_dicount, min_discount,discount_type,discount_amount, GivenDate, CurrentDate,coupon_status;
...
componentDidMount() {
    ...
    geolocator.locate(geooptions, (err, location) => {
      ...
      cookies.set('Geolocation',location.address.city, {path: '/'});
    });
    ...
    $('#brandnamelist').on('change', function(){
        ...
        $('.discountAmountTotallabel').css('display','none')


        
        $('.discountAmountlabel').css('display','none')
    ...
    $('#modelnamelist').on('change', function(){
      ...
      const problemSelections = {
        model: selected,
        location : cookies.get("Geolocation")
      }
      
      var brand  = cookies.get('device-brand')
      if(selected.includes('iPhone') || brand.includes('OnePlus') || brand.includes('Xiaomi')){
        var res;
      axios.post('https://crm.iservice.co.in/website/problems/', problemSelections).then(responsed => {
        ...
        for(var i=0;i<responsed.data.length;i++){
          ...
          if(res.data[i].problem == 'Water Damage'){
             var deviceChecks = '<div class="col-sm-12 col-md-6 col-lg-6"><div data_amount="'+res.data[i].cost+'" id="'+res.data[i].problem+'" data-attr="'+res.data[i].cost+'" class="handleSuggested moreoptionava'+i+'">'+
                              '<p class="individualname">'+res.data[i].problem+'</span><span style="margin-right:6px;" class="individualamount" id="individualamount'+[i]+'"><div class="demo"></div><div class="realamount">₹'+res.data[i].cost+'</div></span></p>'+
                              '<div style="margin-left:6px;max-width:250px;font-size:12px;float:left;margin-top:-20px;color:red;">Final charge to be confirmed after Diagnosis</div>'+
                              ...
                              '<div style="display:none;max-height:10px">'+
                                ...
                                '<div class="btn-row-popup-menu-body">'+
                                  '<div>'+
                                      ...
                                      '<h4 style="color:#2B4162;font-weight:500px;">you need this repair if:</h4>'+
             ...
             var deviceChecks = '<div class="col-sm-12 col-md-6 col-lg-6"><div data_amount="'+res.data[i].cost+'" id="'+res.data[i].problem+'" data-attr="'+res.data[i].cost+'" class="handleSuggested moreoptionava'+i+'">'+
                                  '<p class="individualname">'+res.data[i].problem+'</span><span style="margin-right:6px;" class="individualamount" id="individualamount'+[i]+'"><div class="demo"></div><div class="realamount">₹'+res.data[i].cost+'</div></span></p>'+
                                  '<div style="margin-left:6px;font-size:12px;float:left;margin-top:-20px;color:red">Repair Time : '+res.data[i].time+' Mins </div>'+
          ...
          if(responsed.data[i].old_cost>10){
            $('#individualamount'+i).prepend('<div class="price-wrapper"> <div class="price-slash"></div> <div class="price">'+responsed.data[i].old_cost+'</div> </div>')
          }
        ...
        $(".handleSuggested").on("click", function() {
          ...
          $('.addedproblemamount').empty();
            ...
            if(amountIds.length>=1){
              $('#coupondisable').css('display','none')
            ...
            if(selectedIds.length == 0){
              ...
              $('#coupondisable').css('display','block')
            ...
            if(selectedIds.length> 0  && flag == true){
              flag = true;
              $('#checkoutdisbale').css('display','none')
              $('#coupondisable').css('display','none')

              $('#checkout').css('display','block')
              $('#coupon').css('display','block')

              $('.discountAmountlabel').css('display','inline-block')
              $('.discountAmount').css('display','inline-block')
              $('.invalidcoupon').css('display','none')
            ...
            if(selectedIds.length== 0 && flag != true){
              flag = false;
              $('#checkoutdisbale').css('display','block')
            ...
            if(selectedIds.length > 0){
              ...
              GivenDate = cookies.get('GivenDate')
              CurrentDate = cookies.get('CurrentDate')
              discount_type = cookies.get('discount_type')
              if(discount_check){
                if(GivenDate > CurrentDate){
                  ...
                  if(discount_type == "percentage"){
                    var discount = (discount_amount*cookies.get('total-estimate'))/100
                    var est = cookies.get('total-estimate') - discount;
                      cookies.set('total-estimate', est, {path: '/'});

                    if(discount > 1000){
                      $('.discountAmountlabel').css('display','inline-block')
                      $('.discountAmount').css('display','inline-block')
                      $('.discountAmount').html('1000')

                      $('.discountAmountTotal').html(cookies.get('total-estimate') - discount)
                      $('.discountAmountTotallabel').css('display','inline-block')
                      $('.discountAmountTotal').css('display','inline-block')
                  ...
                  $('.discountAmountlabel').html("Invalid coupon code")
                  $('.discountAmountTotallabel').css('display','none')
                  $('.discountAmountTotal').css('display','none')
                  $('.discountAmountlabel').css('display','none')
                  $('.discountAmount').css('display','none')
                }
              }
  ...
  handlePageRedirect(e) {
    ...
    flag = true;
      ...
      const user_id = {
        ...
        problem:  cookies.get('device-issues'),
        stage: this.state.method
      };
 ...
 handleCoupon(){
    const user_id = {
      ...
      axios({
        method: 'POST',
        url: apilink+'/website/validcoupon/',
        data: user_id
      }).then(response => {
        console.log(response)

        coupon_status = response.data.success;
        ...
        if(coupon_status == "True"){
          ...
          GivenDate = response.data.valid_until
          CurrentDate = new Date().toJSON().slice(0,10)

          cookies.set('GivenDate', GivenDate, {path: '/'});
          cookies.set('CurrentDate', CurrentDate, {path: '/'});
          cookies.set('discount_type', discount_type, {path: '/'});
  ...
  handleOtpBtn(){
    $('.displayOtp').css('display','none');
    $('.phonenumber').css('display','block');
    $('.signinbuttondisbaled').css('display','block');
    $('.enableotp').css('display','none');




    var adlink = localStorage.getItem('ad_url_landing');
    ...
    const createlead = {
      phone: this.state.phone,
      device: cookies.get('device-model'),
      utm_source : localStorage.getItem('utm_source1'),
      utm_medium : localStorage.getItem('utm_medium1'),
      utm_campaign : localStorage.getItem('utm_campaign1'),
    }

      axios.post('https://crm.iservice.co.in/website/createlead/', createlead, axiosConfig).
      then(response => {
        console.log("response")
      }).catch(error => {
          console.log(error);
      ...
      if (response.data.status === 'Successful') {
        ...
        axios({
          ...
          data: dataSignup
            ...
            gtag('event', 'conversion', {'send_to': 'AW-970917228/i4gTCLrmvGoQ7Ir8zgM'});
  ...
  render() {
    const meta = {
      title: 'Apple, OnePlus, Xiaomi Repair Bangalore and Delhi | iService',
      description: "iService is the most trusted multibrand gadget repair service center in India. We're experts in Apple service, OnePlus, Xiaomi & Multibrand laptop repairs.",
      meta: {
        charset: 'utf-8',
        name: {
           ...
           <br />
            ...
            <Col  xs={12} sm={12} md={8} >
              ...
              <div className="vl"></div>
                ...
                <div className="viewcouponbutton">
                  <Col lg={2} md={2} sm={2} xs={2} />
                  <Col lg={6} md={6} sm={6} xs={6} >
                    <TextField type="text" inputStyle={{backgroundColor: '#f0f3f7', paddingLeft: '15px'}} className="signupintputText" hintStyle={{color: '#9b9b9b', zIndex: '9999', paddingLeft: '15px'}} onChange={this.handleCouponInput} underlineShow={false} fullWidth onFocus={this.handleCheckElement} onBlur={this.handleCheckBlurElement} onKeyDown={this.handleSignupEnter}/>
                  </Col>
                    
                  <Col lg={2} md={4} sm={4} xs={2} >
                    <FlatButton  value="paymode" id= "coupondisable" backgroundColor="#9b9b9b"  hoverColor="#3386f4"  className= "center-block" onClick={this.handleCoupon} label="Apply"  disabled="true" labelStyle={{ fontSize: '10px'}} style={{color: '#fff', fontSize: '10px'}} />
                    <FlatButton  value="paymode" id= "coupon" backgroundColor="#3386f4"  hoverColor="#3386f4"  className= "center-block" onClick={this.handleCoupon} label="Apply" labelStyle={{ fontSize: '10px'}}  style={{color: '#fff',display: 'none', fontSize: '10px'}} />
                    <br />
                  </Col>
                ...
                <div  style={{textAlign:'center'}}  xs={12} md={12} lg={12}>
                  <h4 style={{textAlign:'center',display:'inline-block'}}>Total : <div className="totalAmount" style={{display:'inline-block'}}>₹0</div></h4>
                  <br />
                  <h4 className="discountAmountlabel" style={{textAlign:'center',display:'none',color:'red'}}>Discount : <div className="discountAmount" style={{display:'none'}}>₹0</div></h4>
                  <h4 className="invalidcoupon" style={{textAlign:'center',display:'none',color:'red'}} >Invalid Coupon Code</h4>
                  <br />
                  <h4 className="discountAmountTotallabel" style={{textAlign:'center',display:'none'}}>You pay : <div className="discountAmountTotal" style={{display:'none'}}>₹0</div></h4>
                ...
                <div className="viewcartbutton">
                  ...
                  <Col lg={8} md={10} sm={10} xs={10} >
                    ...
                    <br />
                    <FlatButton  value="paymode" id= "checkoutdisbale" backgroundColor="#9b9b9b"  hoverColor="#3386f4"  className= "center-block" onClick={this.handlePageRedirectSkippayment} label="Schedule an Appoinment" disabled="true" labelStyle={{ fontSize: '10px'}} style={{color: '#fff', fontSize: '10px'}} />
                    <FlatButton  value="paymode" id= "checkout" backgroundColor="#3386f4"  hoverColor="#3386f4"  className= "center-block" onClick={this.handlePageRedirectSkippayment} label="Schedule an Appoinment" labelStyle={{ fontSize: '10px'}}  style={{color: '#fff',display: 'none', fontSize: '10px'}} />
                    <br />
                    <br />
                    <br />
                  </Col>
                      ...
                      <Col sm={12} xs={12}>
                        <p className="landing-repair-p-num">
                          ...
                          <p className="vertical-dashed-line-content">
                            Pickup & Drop at Flat 299 Anywhere in Bengaluru city limits.
                          </p>
                      </Col>
                    </Row>
                  </Col>
          ...
          <Dialog
            ...
            <Container>
              ...
              <p className="signInErr">{this.state.invalid}</p>
                ...
                <Row className="phonenumber">
                  <div className="signinbutton signinbuttondisbaled">
                    <p>Sending otp ...</p>
                    <FlatButton backgroundColor="#9b9b9b"  hoverColor="#3386f4" label="Get OTP" disabled="true" fullWidth style={{color: '#fff', marginTop: '50px', minHeight: '55px', borderRadius: '5px', cursor: 'pointer'}} onTouchTap={this.handleSignupOtpBtn}/>
                 </div>
                </Row>
                ...
                <br />
                  <div style={{textAlign: 'center'}}>
                    <sup> I agree to all Terms and Conditions</sup>
                  </div>
                </Row>
